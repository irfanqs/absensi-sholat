...
function normalizeImportHeader(value) { return String(value || "").toLowerCase().replace(/[\s./_-]/g, ""); }

function parseImportedRows(rows) {
  const headerIndex = rows.findIndex((row) => row.some((cell) => ["nama", "n a m a", "l/p", "nis"].includes(String(cell || "").toLowerCase().trim())));
  if (headerIndex < 0) throw new Error("Header tidak ditemukan. Gunakan kolom NIS, Nama, L/P, dan Kelas.");
  ...
  const nameIndex = findColumn("nama", "nama lengkap", "namasiswa", "namamurid");
  ...
  const classText = rows.slice(0, headerIndex).flat().map((cell) => String(cell || "")).join(" ");
  ...
  return rows.slice(headerIndex + 1).map((row) => {
    ...
    const genderValue = String(genderIndex >= 0 ? row[genderIndex] || "" : "").trim().toUpperCase();
    const className = String(classIndex >= 0 ? row[classIndex] || fallbackClass : fallbackClass).trim();
    if (!name || !nis && !genderValue && !className) return null;
    const gender = genderValue === "P" || genderValue.includes("PEREMPUAN") ? "Perempuan" : genderValue === "L" || genderValue.includes("LAKI") ? "Laki-laki" : "";
    const username = nis || name.toLowerCase().replace(/[^a-z0-9]+/g, ".").replace(/^\.|\.$/g, "");
    return { id: createId(), nis, name, gender, className, username, password: "123" };
  }).filter(Boolean);
}
...
function jakartaMinutesNow() {
  const [hour, minute] = new Intl.DateTimeFormat("en-GB", { hour: "2-digit", minute: "2-digit", hourCycle: "h23", timeZone: "Asia/Jakarta" }).format(new Date()).split(":").map(Number);
...
function AdminApp(props) { const { students, attendances, history, view, setView, query, setQuery, classFilter, setClassFilter, filteredStudents, editing, setEditing, onSave, onDelete, onLogout } = props; const [mobileMenuOpen, setMobileMenuOpen] = useState(false); const selectView = (nextView) => { setView(nextView); setMobileMenuOpen(false); }; const present = attendances.length; const absent = students.length - present; return <div className="app-shell"><aside className={mobileMenuOpen ? "menu-open" : ""}><div className="mobile-nav-head"><Logo /><button className="mobile-menu-toggle" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label={mobileMenuOpen ? "Tutup menu" : "Buka menu"}>{mobileMenuOpen ? <X size={22} /> : <List size={22} />}</button></div><nav><NavButton active={view === "dashboard"} onClick={() => selectView("dashboard")} icon={<House size={20} />}>Ringkasan</NavButton><NavButton active={view === "reports"} onClick={() => selectView("reports")} icon={<CalendarBlank size={20} />}>Rekap</NavButton><NavButton active={view === "students"} onClick={() => selectView("students")} icon={<UsersThree size={20} />}>Data murid</NavButton><NavButton active={view === "qr"} onClick={() => selectView("qr")} icon={<QrCode size={20} />}>QR Dzuhur</NavButton><NavButton active={view === "settings"} onClick={() => selectView("settings")} icon={<Gear size={20} />}>Pengaturan</NavButton></nav></aside><main className="admin-main"><header className="admin-header"><div>{view === "dashboard" && <p className="eyebrow">{formatDate()}</p>}<h1>{view === "dashboard" ? "Ringkasan hari ini" : view === "reports" ? "Rekap absensi" : view === "students" ? "Data murid" : view === "qr" ? "QR sholat Dzuhur" : "Pengaturan"}</h1></div></header>{view === "dashboard" && <Dashboard present={present} absent={absent} total={students.length} attendances={attendances} />}{view === "reports" && <ReportPage students={students} history={history} />}{view === "students" && <Students students={filteredStudents} query={query} setQuery={setQuery} classFilter={classFilter} setClassFilter={setClassFilter} setEditing={setEditing} onDelete={onDelete} />}{view === "qr" && <QrPage />}{view === "settings" && <SettingsPage onLogout={onLogout} />}</main>{editing !== null && <StudentModal student={editing} onClose={() => setEditing(null)} onSave={onSave} />}</div>; }
...
function Dashboard({ present, absent, total, attendances }) { return <><section className="metric-grid"><Metric label="Sudah hadir" value={present} detail="Murid terkonfirmasi" tone="green" /><Metric label="Belum hadir" value={absent} detail="Perlu ditinjau" tone="sand" /><Metric label="Total murid" value={total} detail="Dari empat kelas" tone="plain" /></section><section className="attendance-panel"><div className="panel-title"><div><h2>Absensi terbaru</h2><p>Konfirmasi yang masuk hari ini.</p></div><span>{present} murid hadir</span></div>{attendances.length ? <div className="attendance-list">{attendances.map((item) => <div key={item.id}><span className="person-initial">{item.studentName[0]}</span><strong>{item.studentName}</strong><span className="attendance-class">Kelas {item.className}</span><time>{item.time}</time></div>)}</div> : <div className="empty"><CheckCircle size={28} />Belum ada konfirmasi Dzuhur hari ini.</div>}</section></>; }
function ReportPage({ students, history }) { const today = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Jakarta" }).format(new Date()); const [period, setPeriod] = useState("daily"); const [selectedDate, setSelectedDate] = useState(today); const [selectedClass, setSelectedClass] = useState("Semua kelas"); const records = useMemo(() => { const reference = new Date(selectedDate + "T12:00:00"); const day = (reference.getDay() + 6) % 7; const start = new Date(reference); start.setDate(reference.getDate() - day); const end = new Date(start); end.setDate(start.getDate() + 6); const month = selectedDate.slice(0, 7); return history.filter((item) => { const inPeriod = period === "daily" ? item.date === selectedDate : period === "weekly" ? item.date >= start.toISOString().slice(0, 10) && item.date <= end.toISOString().slice(0, 10) : item.date.startsWith(month); return inPeriod && (selectedClass === "Semua kelas" || item.className === selectedClass); }).sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time)); }, [history, period, selectedDate, selectedClass]); const uniqueStudents = new Set(records.map((item) => item.studentId)).size; return <section className="report-panel"><div className="report-controls"><div className="period-toggle"><button className={period === "daily" ? "active" : ""} onClick={() => setPeriod("daily")}>Harian</button><button className={period === "weekly" ? "active" : ""} onClick={() => setPeriod("weekly")}>Mingguan</button><button className={period === "monthly" ? "active" : ""} onClick={() => setPeriod("monthly")}>Bulanan</button></div><label>Tanggal acuan<input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} /></label><label>Kelas<select value={selectedClass} onChange={(event) => setSelectedClass(event.target.value)}><option>Semua kelas</option><option>9A</option><option>9B</option><option>9C</option><option>9D</option></select></label></div><div className="report-summary"><div><span>Murid tercatat</span><strong>{uniqueStudents}</strong></div><div><span>Total konfirmasi</span><strong>{records.length}</strong></div><p>{period === "daily" ? "Daftar murid yang sudah sholat pada tanggal terpilih." : period === "weekly" ? "Daftar konfirmasi sholat selama minggu dari tanggal terpilih." : "Daftar konfirmasi sholat selama bulan dari tanggal terpilih."}</p></div><div className="report-list">{records.length ? records.map((item) => <div key={item.id}><span className="person-initial">{item.studentName[0]}</span><div><strong>{item.studentName}</strong><span>Kelas {item.className}</span></div><time>{new Intl.DateTimeFormat("id-ID", { day: "numeric", month: "short" }).format(new Date(item.date + "T12:00:00"))}<b>{item.time}</b></time></div>) : <div className="empty"><CalendarBlank size={28} />Belum ada absensi pada periode ini.</div>}</div></section>; }
...
function Students({ students, query, setQuery, classFilter, setClassFilter, setEditing, onDelete }) { const [preview, setPreview] = useState(null); function handleFile(event) { const file = event.target.files?.[0]; event.target.value = ""; if (!file) return; const reader = new FileReader(); reader.onload = (loadEvent) => { try { const workbook = XLSX.read(loadEvent.target.result, { type: "array" }); const rows = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]], { header: 1, defval: "" }); const imported = parseImportedRows(rows); setPreview({ fileName: file.name, students: imported, error: imported.length ? "" : "Tidak ada data murid yang bisa dibaca." }); } catch (error) { setPreview({ fileName: file.name, students: [], error: error.message }); } }; reader.readAsArrayBuffer(file); } function confirmImport() { const current = JSON.parse(localStorage.getItem("dzuhur-students") || "[]"); const usernames = new Set(current.map((item) => item.username)); localStorage.setItem("dzuhur-students", JSON.stringify([...current, ...preview.students.filter((item) => !usernames.has(item.username))])); window.location.reload(); } return <section className="students-panel"><div className="table-actions"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari nama atau username" /><select value={classFilter} onChange={(e) => setClassFilter(e.target.value)}><option>Semua kelas</option><option>9A</option><option>9B</option><option>9C</option><option>9D</option></select><label className="file-button secondary"><input type="file" accept=".csv,.xlsx,.xls" onChange={handleFile} />Import Excel/CSV</label><button className="primary compact" onClick={() => setEditing({ name: "", className: "9A", username: "", password: "123" })}><Plus size={18} />Tambah murid</button></div><div className="data-table"><div className="table-head"><span>Nama lengkap</span><span>Kelas</span><span>Username</span><span>Password</span><span>Aksi</span></div>{students.map((student) => <div className="table-row" key={student.id}><strong>{student.name}</strong><span><b className="class-chip">{student.className}</b></span><span>{student.username}</span><span className="password-text">{student.password}</span><span className="row-actions"><button aria-label="Edit murid" onClick={() => setEditing(student)}><PencilSimple size={18} /></button><button aria-label="Hapus murid" onClick={() => onDelete(student.id)}><Trash size={18} /></button></span></div>)}</div>{preview && <ImportPreview preview={preview} onClose={() => setPreview(null)} onConfirm={confirmImport} />}</section>; }
