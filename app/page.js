...
function Logo() { return <div className="brand"><span className="brand-mark"><BookOpen size={22} weight="bold" /></span><span>Sahabat<span>Dzuhur</span></span></div>; }
...
export default function Home() {
  ...
  function login(event) {
    ...
    if (username === "guru" && password === "123") { setUser({ name: "Guru Admin", role: "admin" }); setView("dashboard"); return; }
    const student = students.find((item) => item.username === username && item.password === password);
    if (student) { setUser({ ...student, role: "student" }); setView("dzuhur"); return; }
