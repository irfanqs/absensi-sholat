...
import {
  BookOpen, CalendarBlank, CheckCircle, ClipboardText, DownloadSimple, Gear, House, PencilSimple,
...
function QrPage() { const url = typeof window === "undefined" ? "https://domain-sekolah.id/dzuhur" : `/dzuhur`; const qrRef = useRef(null); function downloadQr() { const canvas = qrRef.current?.querySelector("canvas"); if (!canvas) return; const link = document.createElement("a"); link.download = "qr-absensi-dzuhur.png"; link.href = canvas.toDataURL("image/png"); link.click(); } return <section className="qr-panel"><div><p className="eyebrow">SATU QR PERMANEN</p><h2>Tempelkan QR ini di area sholat.</h2><p>Setelah murid login, QR membuka halaman konfirmasi sholat Dzuhur.</p><code>{url}</code><a className="primary qr-link" href="/dzuhur">Buka halaman Dzuhur</a></div><div className="qr-area"><div ref={qrRef} className="qr-print"><QRCodeCanvas value={url} size={280} bgColor="#ffffff" fgColor="#123d2a" includeMargin /></div><button className="secondary qr-download" onClick={downloadQr}><DownloadSimple size={19} />Download QR</button></div></section>; }
function SettingsPage({ onLogout }) { return <section className="simple-panel"><ClipboardText size={28} /><h2>Pengaturan aplikasi</h2><p>QR permanen mengarah ke halaman Dzuhur. Nanti domain VPS dapat diatur dari bagian ini.</p><div className="settings-signout"><div><strong>Keluar dari akun</strong><span>Anda perlu login kembali untuk mengakses dashboard.</span></div><button className="danger-button" onClick={onLogout}><SignOut size={19} />Keluar</button></div></section>; }
