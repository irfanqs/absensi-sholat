...
import {
  BookOpen, CheckCircle, ClipboardText, DownloadSimple, Gear, House, PencilSimple,
...
function AdminApp(props) { const { students, attendances, view, setView, query, setQuery, classFilter, setClassFilter, filteredStudents, editing, setEditing, onSave, onDelete, onLogout } = props; const present = attendances.length; const absent = students.length - present; return <div className="app-shell"><aside><Logo /><nav><NavButton active={view === "dashboard"} onClick={() => setView("dashboard")} icon={<House size={20} />}>Ringkasan</NavButton><NavButton active={view === "students"} onClick={() => setView("students")} icon={<UsersThree size={20} />}>Data murid</NavButton><NavButton active={view === "qr"} onClick={() => setView("qr")} icon={<QrCode size={20} />}>QR Dzuhur</NavButton><NavButton active={view === "settings"} onClick={() => setView("settings")} icon={<Gear size={20} />}>Pengaturan</NavButton></nav></aside><main className="admin-main"><header className="admin-header"><div><p className="eyebrow">{formatDate()}</p><h1>{view === "dashboard" ? "Ringkasan hari ini" : view === "students" ? "Data murid" : view === "qr" ? "QR sholat Dzuhur" : "Pengaturan"}</h1></div></header>{view === "dashboard" && <Dashboard present={present} absent={absent} total={students.length} attendances={attendances} />}{view === "students" && <Students students={filteredStudents} query={query} setQuery={setQuery} classFilter={classFilter} setClassFilter={setClassFilter} setEditing={setEditing} onDelete={onDelete} />}{view === "qr" && <QrPage />}{view === "settings" && <SettingsPage onLogout={onLogout} />}</main>{editing !== null && <StudentModal student={editing} onClose={() => setEditing(null)} onSave={onSave} />}</div>; }
